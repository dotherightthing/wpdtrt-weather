...
if ( !function_exists( 'wpdtrt_weather_html_image' ) ) {
   ...
   * @since       0.1.0
   */
  function wpdtrt_weather_html_image( $key, $has_enlargement = 0 ) {
    ...
    if ( $has_enlargement ) {
      ...
      if ( isset( $wpdtrt_weather_data[$key]->{'address'} ) ) {
        ...
        $str .= '&zoom=2';
        $str .= '&markers=' . wpdtrt_weather_html_latlng( $key );
        ...
        $str .= '<a href="';
        $str .= $wpdtrt_weather_data[$key]->{'url'};
        $str .= '">';
    ...
    if ( isset( $wpdtrt_weather_data[$key]->{'address'} ) ) {
      ...
      $str .= $wpdtrt_weather_data[$key]->{'thumbnailUrl'};

    }

    $str .='" alt="';

    $str .= wpdtrt_weather_html_title( $key, $has_enlargement );
...
if ( !function_exists( 'wpdtrt_weather_html_latlng' ) ) {
   ...
   * @since       0.1.0
   */
  function wpdtrt_weather_html_latlng( $key ) {
    ...
    if ( isset( $wpdtrt_weather_data[$key]->{'address'} ) ) :
      ...
      $lat = $wpdtrt_weather_data[$key]->{'address'}->{'geo'}->{'lat'};
      $lng = $wpdtrt_weather_data[$key]->{'address'}->{'geo'}->{'lng'};
...
if ( !function_exists( 'wpdtrt_weather_html_title' ) ) {
   ...
   * @since       0.1.0
   */
  function wpdtrt_weather_html_title( $key, $has_enlargement = 0 ) {
    ...
    if ( isset( $wpdtrt_weather_data[$key]->{'address'} ) ) {
      ...
      $str = 'Map showing the co-ordinates ' . wpdtrt_weather_html_latlng( $key );

    // photo - coloured block
    } else {

      $str = $wpdtrt_weather_data[$key]->{'title'};
...
if ( !function_exists( 'wpdtrt_weather_html_date' ) ) {
   ...
   * @since       0.1.0
   */
  function wpdtrt_weather_html_date() {
    ...
    $str = '<p class="wpdtrt-weather-date">Data last updated: ' . date( $wp_date_format, $last_updated ) . '. </p>';
