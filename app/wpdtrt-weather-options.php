...
if ( !function_exists( 'wpdtrt_weather_options_page' ) ) {
   ...
   *       a. The user's selection: 'wpdtrt_weather_datatype',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_weather_data'
  ...
  function wpdtrt_weather_options_page() {
    ...
    if ( isset( $_POST['wpdtrt_weather_form_submitted'] ) ) {
      ...
      if ( $hidden_field === 'Y' ) {
        ...
        $wpdtrt_weather_datatype = esc_html( $_POST['wpdtrt_weather_datatype'] );

        // 2. send user preferences to API to get results
        $wpdtrt_weather_data = wpdtrt_weather_get_data(
          $wpdtrt_weather_datatype
        );

        // 3a. store user preferences in options object
        $wpdtrt_weather_options['wpdtrt_weather_datatype'] = $wpdtrt_weather_datatype;

        // 3b. store API results in options object
        $wpdtrt_weather_options['wpdtrt_weather_data'] = $wpdtrt_weather_data;

        // 3c. store timestamp in options object
        $wpdtrt_weather_options['last_updated'] = time(); // UNIX timestamp for the current time
